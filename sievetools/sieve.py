...
def get_primes(nmax, method = "slow"):
    if method == "slow":
        return get_primes1(nmax)
    else:
        return get_primes2(nmax)


def proportion_primes(all_nmax, which_function):
    """
    This function evaluates the proportion of primenumbers as we
    increase the search window.
    
    Parameters: 
        nmax: takes the maximum number of numbers allowed in the list
        proportion: 
    
    """
    all_proportions = []
    for nmax in all_nmax:
        all_primes = get_primes(nmax, method = which_function)
